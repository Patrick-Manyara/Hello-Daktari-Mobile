...
export default function ForumScreen({ route, navigation }) {
  ...
  const navigateToSingleForum = (forum) => {
    navigation.navigate("ForumDetailsScreen", {
      forum: forum,
      doctors: doctors,
    });
  };
  ...
  function handleCategoryClick(categoryKey) {
    ...
    if (categoryKey === "all") {
      ...
      if (categoryKey === "popular") {
        ...
        // Filter forums where user_id is equal to user.user_id
        filteredForums = initialForums.filter(
          (forum) => forum.user_id === user.user_id
        );
      }
      ...
      >
        ...
        <View style={{ width: "80%", marginLeft: 10 }}>
          ...
          <NormalText styleProp={{ color: "#fff" }}>
            Welcome to our patient forum. Use the refresh button on the right or add button below.
          </NormalText>
        </View>
        <IconButton
          ...
          <View>
            ...
            <View>
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                <PrimaryButton onPress={() => openModal()} styleProp={styles.btnStyle}>
                  Add New
                </PrimaryButton>
                <InputHybrid
